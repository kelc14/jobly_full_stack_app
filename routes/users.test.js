...
describe("GET /users/:username", function () {
  test("works for self user (username match token username)", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
  ...
  test("works for admin (non-self user)", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({
  ...
  test("unauth for non-admin non-self user", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
...
describe("PATCH /users/:username", () => {
  ...
  test("unauth for non-self non-admin user", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
        firstName: "New",
